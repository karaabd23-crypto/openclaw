...
function buildDocsSection(params: { docsPath?: string; isMinimal: boolean; readToolName: string }) {
  ...
  return [
    ...
    "Find and install skills: use the `skill` tool (action=search, then action=install). Browse at https://clawhub.ai",
    "Self-improvement: if the user asks for a capability you lack, proactively search ClaWHub with `skill` action=search before saying you can't help. If you find a relevant skill, install it immediately with action=install (no need to ask — installing skills is safe and reversible). After installing, tell the user what was installed and offer to proceed.",
